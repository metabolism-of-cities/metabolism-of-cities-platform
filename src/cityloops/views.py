...
def team(request):
    if request.user.id == 1 and "import" in request.GET:
        messages.success(request, "Records imported")
        existing = LibraryItem.objects.filter(meta_data__eurostat_code__isnull=False)
        existing.delete()
        list = EurostatDB.objects.filter(is_approved=True, is_duplicate=False)
        e = Tag.objects.get(pk=7)
        mfa = Tag.objects.get(pk=85)
        sdg = Tag.objects.get(pk=1083)
        ce = Tag.objects.get(pk=185)
        e.parent_tag_id=965
        e.name = "Eurostat dataset"
        e.save()
        existing = LibraryItem.objects.filter(tags=e)
        countries_a = ReferenceSpace.objects.filter(pk__in=[14293, 14294, 14504, 14505, 14508, 14297, 14298, 14511, 14512, 14301])
        countries_b = ReferenceSpace.objects.filter(pk__in=[14555, 14565, 14568, 14513, 14557, 14572, 34671])
        countries_c = ReferenceSpace.objects.filter(pk__in=[14498, 14499, 14519, 14502, 14526, 12036])
        ...
        for each in list:
            ...
            if title in names:
                ...
                names.append(title)
            if each.notes:
                description = each.notes + "\n\n"
            else:
                description = ""
            if not each.has_no_meta_data:
                description += f"Meta information on this dataset [can be found here](https://ec.europa.eu/eurostat/cache/metadata/en/{each.code}_esms.htm)."
            info = LibraryItem.objects.create(
                name = title,
                author_list = "Eurostat",
                url = f"https://appsso.eurostat.ec.europa.eu/nui/show.do?dataset={each.code}&lang=en",
                open_access = True,
                type_id=10,
                license_id=2,
                meta_data = {
                    "eurostat_id": each.id,
                    "eurostat_code": each.code,
                    "eurostat_type": each.type,
                },
                description = each.notes
            )
            info.tags.add(e)
            ...
            for space in each.spaces.all():
                each.spaces.add(space)
            if "env_ac_mfa" in each.code:
                info.tags.add(mfa)
            if "sdg" in each.code:
                info.tags.add(sdg)
            if "cei_" in each.code:
                info.tags.add(ce)
            if "med_" in each.code:
                for c in countries_a:
                    info.spaces.add(c)
            if "cpc_" in each.code:
                for c in countries_b:
                    info.spaces.add(c)
            if "enpr_" in each.code:
                for c in countries_c:
                    info.spaces.add(c)

    if request.user.id == 1 and "nuts" in request.GET:
        nuts2 = Tag.objects.get(pk=816)
        nuts3 = Tag.objects.get(pk=817)
        list = LibraryItem.objects.filter(meta_data__eurostat_code__isnull=False).filter(Q(tags=nuts2)|Q(tags=nuts3)).exclude(meta_data__nuts_complete__isnull=False)
        list = list[:50]
        all_nuts2 = ReferenceSpace.objects.filter(source_id=584147)
        all_nuts3 = ReferenceSpace.objects.filter(source_id=584490)
        msg = ''
        for info in list:
            if nuts2 in info.tags.all():
                for each in all_nuts2:
                    info.spaces.add(each)
            elif nuts3 in info.tags.all():
                for each in all_nuts3:
                    info.spaces.add(each)
            info.meta_data["nuts_complete"] = True
            info.save()
            msg += str(info)
