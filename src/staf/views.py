...
def referencespaces_validator(request):
    spaces = None
    spaces_names = None
    hits = None

    if "load_fao" in request.GET:
        
        check = MaterialCatalog.objects.filter(name="FAO Crops and livestock products")
        if not check:
            catalog = MaterialCatalog.objects.create(name="FAO Crops and livestock products", url="https://www.fao.org/faostat/en/#data")
            products = {
    ...
    if request.method == "POST":
        ...
        for each in spaces:
            if not each:
                spaces.remove(each)
        spaces = [i.strip() for i in spaces]
        if "source" in request.POST:
            source = LibraryItem.objects.get(pk=request.POST.get("source"))
            spaces_names = source.imported_spaces.values_list("name", flat=True)
            spaces_names = list(spaces_names)
        else:
            check = spaces[0]
            hits = ReferenceSpace.objects.filter(name=check, source__isnull=False)
    context = {
        "spaces": spaces,
        "spaces_names": spaces_names,
        "hits": hits,
    }
    return render(request, "staf/referencespaces.validator.html", context)
...
def materials(request, id=None, catalog=None, project_name=None, edit_mode=False):
    ...
    if id:
        ...
        parent_elements = Material.objects.filter(tree_node__ancestors__contains=[id])
        parent_elements = MaterialTree.objects.get(node_id=id)
        p(parent_elements.root)
        p(parent_elements.ancestors)
        a = Material.objects.filter(pk__in=parent_elements.ancestors).order_by("tree_node__ancestors")
        p(a)
    else:
        if not catalog:
            ...
            if material_code_column:
                ...
                for each in materials:
                    ...
                    if each not in material_list:
                        try:
                            check = Material.objects.get(catalog_id=info.get_associated_materials_catalog, code=each)
